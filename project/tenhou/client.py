...
class TenhouClient(Client):
    ...
    def __init__(self, logger, bot_config=None):
        super().__init__(bot_config=bot_config)
        self.logger = logger
                ...
                if self.decoder.is_opened_set_message(message):
                    ...
                    if isinstance(self.player, MortalPlayer):
                        ...
                        if meld.type == MeldPrint.PON:
                            pon_tiles = list(meld.tiles)
                            pon_tiles.remove(meld.called_tile)
                            self.player.events.append(mortal_helpers.pon(
                                player_id=meld.who,
                                from_whom=meld.from_who,
                                tile=mortal_helpers.convert_tile_to_mortal(tile_136=meld.called_tile),
                                pon_tiles=[mortal_helpers.convert_tile_to_mortal(tile_136=t) for t in pon_tiles],
                            ))
                        ...
                        if self.player.events[-1]["type"] == "dora":
                            if self.player.events[-2]["type"] == "tsumo" and self.player.events[-2]["actor"] == player_seat:
                                if self.player.events[-3]["type"] in {"kakan", "daiminkan"} and self.player.events[-3]["actor"] == player_seat:
                                    ...
                                    previous_kan_dora_event = self.player.events[-1]
                                    self.player.events.pop()
